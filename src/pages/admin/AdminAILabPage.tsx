...
export default function AdminAILabPage() {
  ...
  const fetchProducts = useCallback(async (q = '') => {
    try {
      const { data } = await apiClient.get('admin/products/', { params: { search: q } });
  ...
  const handleRunTest = async () => {
    ...
    try {
      ...
      const { data: result } = await apiClient.post(`admin/ai-tests/${testObj.id}/run_test/`);
      ...
      console.error(e);
      showToast(e.response?.data?.error || "AI Testida xatolik yuz berdi", "error");
    } finally {
      setRunning(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b border-slate-200/60">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">AI Laboratory</h1>
          </div>
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest ml-14">Vizualizatsiya pipeline'ni test qilish va debug</p>
        </div>
          ...
          >
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-sm space-y-8">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-black text-slate-800 flex items-center gap-3">
                      <span className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-500">1</span>
                      Eshik tanlash
                    </h3>
                    {selectedProduct && (
                        <button onClick={() => setSelectedProduct(null)} className="text-[10px] font-bold text-red-500 uppercase tracking-widest hover:underline">Almashtirish</button>
                    )}
                  </div>
                  ...
                  {!selectedProduct ? (
                    ...
                    >
                        ...
                        <p className="text-sm font-bold text-slate-500 group-hover:text-sky-600">Katalogdan eshikni tanlang</p>
                    </button>
                  ) : (
                    <div className="flex items-center gap-6 p-4 bg-slate-50 rounded-[20px] border border-slate-100">
                        <div className="w-20 h-28 bg-white rounded-xl border border-slate-200 p-2 flex items-center justify-center overflow-hidden flex-shrink-0">
                            <img 
                                src={selectedProduct.image?.startsWith('http') ? selectedProduct.image : `${MEDIA_BASE}${selectedProduct.image}`} 
                                className="max-w-full max-h-full object-contain mix-blend-multiply" 
                                alt={selectedProduct.name} 
                            />
                        </div>
                        <div>
                            <p className="text-base font-black text-slate-800 mb-1">{selectedProduct.name}</p>
                            <span className={cn(
                                "inline-block px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider",
                                selectedProduct.ai_status === 'completed' ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                            )}>
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-black text-slate-800 flex items-center gap-3">
                    <span className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-500">2</span>
                    Xona rasmi
                  </h3>
                  ...
                  <div className="relative group">
                    ...
                    {!roomPreview ? (
                        <div className="w-full h-64 border-2 border-dashed border-slate-100 rounded-[24px] flex flex-col items-center justify-center bg-slate-50/50 group-hover:border-sky-300 group-hover:bg-sky-50 transition-all">
                            <Upload className="text-slate-300 group-hover:text-sky-500 mb-4 transition-all" size={32} />
                            <p className="text-sm font-bold text-slate-400 group-hover:text-sky-600">Rasmni yuklang yoki bu yerga tashlang</p>
                            <p className="text-[10px] text-slate-300 font-bold uppercase mt-2">PNG, JPG gacha 10MB</p>
                        </div>
                    ) : (
                        <div className="w-full h-80 rounded-[24px] overflow-hidden relative shadow-lg">
                            <img src={roomPreview} className="w-full h-full object-cover" alt="Room Preview" />
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">
                                <p className="text-white text-xs font-black uppercase tracking-widest">Rasmni almashtirish</p>
                            </div>
                        </div>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 group cursor-help">
                        ...
                        <span className="text-[10px] font-bold text-slate-300 group-hover:text-slate-500 uppercase tracking-widest transition-colors">Yordam</span>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
                        ...
                        <ul className="space-y-6">
                            <li className="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-slate-400 leading-relaxed font-medium">Test jarayoni 10-30 soniya olishi mumkin. Brauzerni yopmang.</p>
                                </div>
                            </li>
                            <li className="flex gap-4">
                                <div className="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-slate-400 leading-relaxed font-medium">Siz turli xil yorug'lik va burchakdagi xonalarni sinab ko'rishingiz mumkin.</p>
                                </div>
                            </li>
                        </ul>
                    </div>
                ...
                <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-sm">
                    <h4 className="text-[11px] font-black uppercase tracking-widest text-[#0067a5] mb-6">Oxirgi Testlar</h4>
                        ...
                        {tests.slice(0, 3).map(t => (
                            <div key={t.id} className="flex items-center gap-4 group cursor-pointer" onClick={() => setActiveTab('history')}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[11px] font-black text-slate-700 truncate">{t.door_details.name}</p>
                                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight mt-0.5">{new Date(t.created_at).toLocaleTimeString()}</p>
                                </div>
                                <ChevronRight size={14} className="text-slate-300 group-hover:text-slate-600 transition-all group-hover:translate-x-1" />
                            </div>
                        ))}
                        {tests.length === 0 && <p className="text-center text-[10px] text-slate-300 font-bold uppercase py-4">Tarix bo'sh</p>}
                    </div>
                </div>
            </div>
          </motion.div>
          ...
          >
            {loading && tests.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-32 text-slate-300">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-slate-400">Tarix yuklanmoqda...</p>
                </div>
            ) : tests.length === 0 ? (
                <div className="bg-white rounded-[40px] border border-slate-100 p-24 text-center">
                    ...
                    <h5 className="text-lg font-black text-slate-400 mb-2">Siz hali test amalga oshirmadingiz</h5>
                    <button onClick={() => setActiveTab('create')} className="text-sky-600 font-black text-xs uppercase tracking-widest hover:underline">Laboratoriyani boshlash</button>
                </div>
            ) : (
                            ...
                            <div className="aspect-video relative overflow-hidden bg-slate-50">
                                {t.result_image ? (
                                    ...
                                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-4">
                                        <Clock size={32} />
                                        <p className="text-[10px] font-black uppercase tracking-widest">Natija kutilmoqda</p>
                                    </div>
                                )}
                                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur shadow-xl rounded-xl px-3 py-1.5 flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black text-slate-800">#{t.id} TEST</span>
                                </div>
                                <button 
      ...
      {showProductPicker && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[80] flex items-center justify-center p-4" onClick={() => setShowProductPicker(false)}>
              <motion.div 
              ...
              >
                  <div className="p-8 border-b border-slate-100">
                      <div className="flex items-center justify-between mb-6">
                          <h4 className="text-xl font-black text-slate-900">Eshik tanlash</h4>
                          <button onClick={() => setShowProductPicker(false)} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition-all">
                              <Trash2 size={18} className="rotate-45" /> {/* Just an X icon trick */}
                          </button>
                      </div>
                      ...
                      {products.map(p => (
                          ...
                          >
                              <div className="w-14 h-20 bg-white rounded-lg border border-slate-100 p-1 flex items-center justify-center shadow-sm">
                                  <img src={p.image?.startsWith('http') ? p.image : `${MEDIA_BASE}${p.image}`} className="max-w-full max-h-full object-contain mix-blend-multiply" alt="p" />
                              </div>
                              <div className="text-left flex-1 min-w-0">
                                  <p className="text-sm font-black text-slate-800 mb-1 group-hover:text-sky-600 transition-colors">{p.name}</p>
                                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{p.ai_status}</span>
                              </div>
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            {toast.type === 'success' ? (
              <div className="w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center flex-shrink-0"><CheckCircle2 size={18} className="text-white" /></div>
            ) : (
              <div className="w-8 h-8 bg-red-500 rounded-full flex items-center justify-center flex-shrink-0"><AlertCircle size={18} className="text-white" /></div>
            )}
